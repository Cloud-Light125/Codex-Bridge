...
```text
artifacts\win-x64-1.1.0
```

完整安装包：

```text
artifacts\win-x64-1.1.0\CloudLight-CodexBridge-Setup-1.1.0-win-x64.exe
```

便携版可以直接运行：

```text
artifacts\win-x64-1.1.0\CloudLight.CodexBridge.exe
```
...
```powershell
.\scripts\build.ps1 -Version 1.1.0
```

脚本输出到 `artifacts\win-x64-1.1.0`，并拒绝覆盖已存在的版本目录。构建不会执行 Git 提交、标签或推送。
