...
```text
artifacts\win-x64-1.2.4
```

完整安装包：

```text
artifacts\win-x64-1.2.4\CloudLight-CodexBridge-Setup-1.2.4-win-x64.exe
```

便携版可以直接运行：

```text
artifacts\win-x64-1.2.4\CloudLight.CodexBridge.exe
```
...
```powershell
.\scripts\build.ps1 -Version 1.2.4
```

脚本输出到 `artifacts\win-x64-1.2.4`，并拒绝覆盖已存在的版本目录。构建不会执行 Git 提交、标签或推送。
