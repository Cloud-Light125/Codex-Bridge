...
```text
artifacts\win-x64-1.2.2
```

完整安装包：

```text
artifacts\win-x64-1.2.2\CloudLight-CodexBridge-Setup-1.2.2-win-x64.exe
```

便携版可以直接运行：

```text
artifacts\win-x64-1.2.2\CloudLight.CodexBridge.exe
```
...
脚本输出到 `artifacts\win-x64-1.2.2`，并拒绝覆盖已存在的版本目录。构建不会执行 Git 提交、标签或推送。
